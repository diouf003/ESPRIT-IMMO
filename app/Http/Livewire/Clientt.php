...
class Clientt extends Component
{
    ...
    public function render()
    {
        ...
        return view('livewire.tableau.index', [
    ...
    public function deleteArticle(Article $article)
    {
        ...
        if (count($article->article_propriete) > 0) return;

        $article->article_propriete()->where("article_id", $article->id)->delete();




        if (count($article->Tarifications) > 0) return;
        ...
        $this->dispatchBrowserEvent("showSuccessMessage", ["message" => "Logement supprimé avec succès!!"]);
    }
}
